...
class DataLoader():
    # Loads images, transform them into tensors, store tensors as mini-batches
    def __init__(self, img_path, target_path, category_list, split_size,
                 batch_size, load_size):
        """
            img_path - The path to the image folder
            target_path - The path to the json file containing the image labels
            category_list - List of all the class labels
            split_size - Amount of grid cells
            batch_size - Batch size
            load_size - Amount of batches which are loaded in one call
        """
        ...
        self.img_files = []  # contain the remaining image names from the folder
        self.target_files = []  # contain the json elements with the ground-truth labels

        self.data = []  # contain tuples where each tuple contains a mini-batch of img and target tensors
        self.img_tensors = []  # temporary store image tensors from a single batch
        self.target_tensors = []  # temporary store target tensors from a single batch
    ...
    def LoadFiles(self):
        self.img_files = listdir(self.img_path)
        f = open(self.target_path)
    ...
    def extract_image(self):
        ...
        global img
        img = Image.open(self.img_path + f)
